...
// 1. Logika Nonaktifkan/Arsip (Soft Delete)
...
// 3. Logika HAPUS PERMANEN (Hard Delete) - VERSI AMAN
if (isset($_GET['hard_delete'])) {
    ...
    try {
        ...
        // Jika produk ini ada di tabel order_items (pernah dibeli), database akan MENOLAK (Error)
        $stmt_delete = $db->prepare("DELETE FROM products WHERE id = ? AND is_active = 0");
        $stmt_delete->execute([$id]);

        if ($stmt_delete->rowCount() > 0) {
            // Jika berhasil dihapus dari DB (berarti belum pernah laku), hapus file gambar
        ...
        // Error 23000 (Foreign Key Constraint) artinya produk ini terkait data pesanan
        if ($e->getCode() == '23000') {
...
// Ambil semua produk (Admin perlu melihat semua, aktif maupun nonaktif)
...
<?php if (isset($_GET['status'])): ?>
    <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4" role="alert">
        ...
        if ($_GET['status'] == 'deleted') echo "Produk berhasil dihapus secara permanen.";
        // Pesan Error jika produk tidak bisa dihapus karena sudah laku
        if ($_GET['status'] == 'fk_error') echo "<strong>Gagal menghapus:</strong> Produk ini ada dalam riwayat pesanan. Silakan nonaktifkan saja.";
        ?>
    </div>
<?php endif; ?>
        ...
        <tbody>
            <?php foreach ($products as $product): ?>
                <tr class="border-b <?= $product['is_active'] ? '' : 'bg-gray-50 opacity-75' ?>"> 
                    ...
                    <td class="p-4 text-sm text-gray-600"><?= htmlspecialchars($product['category'] ?? '-') ?></td>
                    ...
                    <td class="p-4 whitespace-nowrap">
                        ...
                        <?php if ($product['is_active']): ?>
                            ...
                            <span class="text-gray-300 cursor-not-allowed" title="Nonaktifkan produk terlebih dahulu untuk menghapus permanen.">Hapus</span>
                        <?php else: ?>
                            ...
                            <a href="products.php?hard_delete=<?= $product['id'] ?>" class="text-red-600 hover:text-red-800" onclick="return confirm('Yakin ingin MENGHAPUS PERMANEN? Pastikan produk ini belum pernah terjual.')">Hapus Permanen</a>
                        <?php endif; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
